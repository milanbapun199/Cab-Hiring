...
public class SourceDestination extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet SourceDestination</title>");            
            out.println("</head>");
            out.println("<body>");
            
            TravellerDao dao=new TravellerDao(ConnectionProvider.getConnection());
            ArrayList list1= dao.getLocation();
            request.setAttribute("loc", list1);
            RequestDispatcher dispatcher=request.getRequestDispatcher("travelling_request.jsp");
